...
const boundingBoxVN = {
  HN:     [105.7700, 20.9600, 105.8800, 21.0500, 0.0008],  // Hà Nội
  TPHCM:  [106.6200, 10.7100, 106.7500, 10.8300, 0.005],  // TP. Hồ Chí Minh
  HUE:    [107.5200, 16.4200, 107.6500, 16.5100, 0.005],  // Huế
...
async function getRandomMapillaryImage() {
  ...
  const radius = 50;
  const limit = 50;
  const url = `https://graph.mapillary.com/images?access_token=${accessToken}&fields=id,thumb_original_url,geometry,is_pano&lat=${randomLat.toFixed(6)}&lng=${randomLng.toFixed(6)}&radius=${radius}&limit=${limit}&is_pano=1`;

  try {
    console.log('Fetching panoramic image...');
    const res = await fetch(url);
    const data = await res.json();
    if (data.data && data.data.length > 0 && data.data.some(img => img.is_pano)) {
